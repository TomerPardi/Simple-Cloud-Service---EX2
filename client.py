...
class Client:
    ...
    def handle_new_pc(self, ID):
        self.__sock.send(ID)
        string = b""
        while string != b"got id": string += self.__sock.recv(64)

        self.__sock.send(str.encode(self.__sub_id))
        string = ""
        while string != b"got sub id": string += self.__sock.recv(64)
    ...
    def handle_new_client(self):
        ...
        # string = b""
        # while string != b"got socket name": string += self.__sock.recv(64)

        self.__sock.send(str.encode(self.__sub_id))
        string = b""
        while string != b"got sub id": string += self.__sock.recv(64)

        while len(self.__id) < 128: self.__id += self.__sock.recv(64).decode()
